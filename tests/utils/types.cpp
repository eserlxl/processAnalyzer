...
TEST(ParseIntegerNoThrowTest, BasicFunctionality) {
    EXPECT_EQ(utils::parseInteger<int>("123", 10), 123);
    EXPECT_EQ(utils::parseInteger<int>("-456", 10), -456);
    EXPECT_EQ(utils::parseInteger<long>("-456", 10), -456L);
    EXPECT_EQ(utils::parseInteger<short>("123", 10), 123);
    EXPECT_EQ(utils::parseInteger<unsigned char>("255", 10), 255);
    EXPECT_EQ(utils::parseInteger<uint32_t>("4294967295", 10), 4294967295U);
}
...
TEST(ParseIntegerNoThrowTest, BaseVariations) {
    ...
    EXPECT_EQ(utils::parseInteger<int>("0b10110", 2), std::nullopt); // 'b' is not a valid binary digit
}


TEST(ParseIntegerNoThrowTest, EdgeCases) {
    EXPECT_EQ(utils::parseInteger<int>("", 10), std::nullopt);
    EXPECT_EQ(utils::parseInteger<int>("   ", 10), std::nullopt);
    EXPECT_EQ(utils::parseInteger<int>("123a", 10), std::nullopt);
    EXPECT_EQ(utils::parseInteger<int>("a123", 10), std::nullopt);
    EXPECT_EQ(utils::parseInteger<int>("0", 10), 0);
    ...
    EXPECT_EQ(utils::parseInteger<int>(numberSv, 10), 12345);
}

TEST(ParseIntegerNoThrowTest, TypeLimits) {
    // Int
    EXPECT_EQ(utils::parseInteger<int>(std::to_string(std::numeric_limits<int>::max()), 10), std::numeric_limits<int>::max());
    EXPECT_EQ(utils::parseInteger<int>(std::to_string(std::numeric_limits<int>::min()), 10), std::numeric_limits<int>::min());

    // Unsigned Char
    EXPECT_EQ(utils::parseInteger<unsigned char>("255", 10), 255);
    EXPECT_EQ(utils::parseInteger<unsigned char>("0", 10), 0);
}
...
TEST(ParseIntegerNoThrowTest, OverflowAndUnderflow) {
    ...
    EXPECT_EQ(utils::parseInteger<int>(posOverflowStr, 10), std::nullopt);
    ...
    EXPECT_EQ(utils::parseInteger<int>(negOverflowStr, 10), std::nullopt);

    // Unsigned Char overflow
    EXPECT_EQ(utils::parseInteger<unsigned char>("256", 10), std::nullopt);
    EXPECT_EQ(utils::parseInteger<unsigned char>("-1", 10), std::nullopt);
