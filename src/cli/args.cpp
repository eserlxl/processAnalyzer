...
void printUsage() {
    std::cout << "Usage: processAnalyzer <command> [options]\n"
              ...
              << "  summary                  Aggregate the process population (count, by-state, zombies, thread/memory totals)\n"
              << "\nTop command options:\n"
              ...
              << "  --field <NAME>           Emit one column's raw value per process, one per line (no header/quoting;\n"
              << "                           for scripting). Same field names as --columns.\n"
